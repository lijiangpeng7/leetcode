<template>
    <div>
        ...
        <ul>
            <li
                v-for="(item) in rightCheckedData"
                :key="item.value"
            >
...
export default {
    ...
    methods: {
        //递归处理树状结构
        recursionFormat(data, path = '') {
            for (let i = 0; i < data.length; i++) {
                let nodData = data[i];
                nodData.path =
                    path == '' ? nodData.id : path + '-' + nodData.id;
                if (nodData.children && nodData.children.length > 0) {
                    this.recursionFormat(nodData.children, nodData.path);
                } else {
                    delete nodData.children;
                }
            }
        },
        ...
        getCheckedDataById(children, arrId) {
            let id = arrId.shift();
            let findIdx = children.findIndex(
                (entry) => entry.id === parseInt(id)
            );
            if (arrId.length > 0) {
                return this.getCheckedDataById(
                    children[findIdx].children,
                    arrId
                );
            } else {
